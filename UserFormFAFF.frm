VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} UserFormF 
   Caption         =   "T&TDG FAFF Database Family Management"
   ClientHeight    =   10590
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   22725
   OleObjectBlob   =   "UserFormFAFF.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "UserFormF"
...
Private Sub cboCate_Change()
    Worksheets("F").Range("C2") = Me.cboCate.Value
    Range("D2").Select
    ActiveCell.FormulaR1C1 = "=IFERROR(VLOOKUP(RC[-1],R[2]C[-1]:R[1000]C,2,FALSE),"""")"
    Me.txtACate.Value = Worksheets("F").Range("D2")
End Sub
...
Private Sub cboSub1_Change()
    Worksheets("F").Range("G2") = Me.cboSub1.Value
    Range("H2").Select
    ActiveCell.FormulaR1C1 = _
        "=IFERROR(VLOOKUP(RC[-1],R[2]C[-1]:R[1000]C,2,FALSE),"""")"
    Me.txtASub1.Value = Worksheets("F").Range("H2")
End Sub

Private Sub cboSub2_Change()
    Worksheets("F").Range("I2") = Me.cboSub2.Value
    Range("J2").Select
    ActiveCell.FormulaR1C1 = _
        "=IFERROR(VLOOKUP(RC[-1],R[2]C[-1]:R[1000]C,2,FALSE),"""")"
    Me.txtASub2.Value = Worksheets("F").Range("J2")
End Sub

Private Sub cboType_Change()
    Worksheets("F").Range("E2") = Me.cboType.Value
    Range("F2").Select
    ActiveCell.FormulaR1C1 = _
        "=IFERROR(VLOOKUP(RC[-1],R[2]C[-1]:R[1000]C,2,FALSE),"""")"
    Me.txtAType.Value = Worksheets("F").Range("F2")
End Sub
...
Else
   ...
   Dim NewFile As String
    ...
    OldFile = Worksheets("F").Range("AV5")
    NewFile = Worksheets("F").Range("AV4")
Existed:
    If Worksheets("F").Range("AV4").Value = Worksheets("F").Range("AV5").Value Then
...
Else
  ...
  lrP = Worksheets("F").Range("P" & Rows.Count).End(xlUp).Row
  i = Worksheets("F").Range("Y4:Y" & lrP).Find(What:=Me.txtName, LookIn:=xlValues).Row - 4
Call del
End If
...
lrB = Worksheets("F").Range("B" & Rows.Count).End(xlUp).Row
Set findvalue = Worksheets("F").Range("K4:K" & lrB).Find(What:=Me.txtName, LookIn:=xlValues)
findvalue.Value = ""
...
'1. SortData
   lrB = Worksheets("F").Range("B" & Rows.Count).End(xlUp).Row
    ...
    ActiveWorkbook.Worksheets("F").Sort.SortFields.Add Key:=Range("B4:B" & lrB), _
        SortOn:=xlSortOnValues, Order:=xlDescending, DataOption:=xlSortNormal

    ActiveWorkbook.Worksheets("F").Sort.SortFields.Add Key:=Range("K4:K" & lrB), _
  ...
  ''''2. Sort Outdata
  lrP = Worksheets("F").Range("P" & Rows.Count).End(xlUp).Row
    Worksheets("F").Range("P3:AA" & lrP).Select
    ActiveWorkbook.Worksheets("F").Sort.SortFields.Clear
    ActiveWorkbook.Worksheets("F").Sort.SortFields.Add Key:=Range("P4:P" & lrP), _
        SortOn:=xlSortOnValues, Order:=xlDescending, DataOption:=xlSortNormal
 
    ActiveWorkbook.Worksheets("F").Sort.SortFields.Add Key:=Range("Y4:Y" & lrP), _
        SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    With ActiveWorkbook.Worksheets("F").Sort
...
''*xet xem da co name bi trung
  ...
  lrB = Worksheets("F").Range("B" & Rows.Count).End(xlUp).Row
  Set findName = Worksheets("F").Range("K4:K" & lrB).Find(What:=Me.txtName, LookIn:=xlValues)
...
Dim findvalue As Range
lrB = Worksheets("F").Range("B" & Rows.Count).End(xlUp).Row
Set findvalue = Sheets("F").Range("M4:M" & lrB).Find(What:=Me.txtID, LookIn:=xlValues)
...
Call cmdSearch_Click
 ...
 lrP = Worksheets("F").Range("P" & Rows.Count).End(xlUp).Row
 On Error Resume Next
 i = Worksheets("F").Range("Y4:Y" & lrP).Find(What:=Me.txtPreview, LookIn:=xlValues).Row - 4
...
Else
    Dim findvalue As Range
  On Error GoTo Existed
    lrAQ = Worksheets("F").Range("AQ" & Rows.Count).End(xlUp).Row
    ...
    Else
      Dim OldFile As String
      Dim NewFile As String
      Worksheets("F").Range("AU4").Value = Me.txtEditFile.Value 'dien ten file moi
      Worksheets("F").Range("AU5").Value = Me.txtEditFile.Value 'dien ten file moi
      OldFile = Worksheets("F").Range("AV2")
      NewFile = Worksheets("F").Range("AV3")
Existed:
    If Worksheets("F").Range("AV2").Value = Worksheets("F").Range("AV3").Value Then
...
''Xoa Get list file cu
    Worksheets("F").Range("AQ:AQ").ClearContents
    Dim myPath As String
    myPath = Me.txtPath.Text
    If myPath <> "" Then
...
'*Sort va Filter du lieu trc khi dien
    ...
    lrAQ = Worksheets("F").Range("AQ" & Rows.Count).End(xlUp).Row
    ActiveWorkbook.Worksheets("F").Sort.SortFields.Clear
    ActiveWorkbook.Worksheets("F").Sort.SortFields.Add Key:=Range("AQ4:AQ" & lrAQ), _
        SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    With ActiveWorkbook.Worksheets("F").Sort
    ...
    'ActiveWorkbook.Worksheets("F").Sort.SortFields.Add2 Key:=Range("AQ4"), _
        'SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
'*Dien du lieu vao Listrow
   ListBox2.List = Worksheets("F").Range("AP4:AQ" & lrAQ).Value
...
End If
'MsgBox (Get_Path)
Worksheets("F").Cells(1, 48).Value = get_Path
...
On Error GoTo errHandler:
 ...
 Worksheets("F").Range("O2") = Me.cboSelect.Value
 Worksheets("F").Range("O3") = Me.txtSearch.Value
lrB = Worksheets("F").Range("B" & Rows.Count).End(xlUp).Row
 Worksheets("F").Range("B3:M" & lrB).AdvancedFilter Action:=xlFilterCopy, _
 CriteriaRange:=Range("F!Criteria"), _
 CopyToRange:=Range("F!Extract"), Unique:=False
 ''tim dong cuoi cua du lieu ket qua de dien vao listrow
 lrP = Worksheets("F").Range("P" & Rows.Count).End(xlUp).Row
 ListBox1.RowSource = Worksheets("F").Range("P4:AA" & lrP).Address
 Me.txtMax.Value = Worksheets("F").Range("N1")
 Me.txtNumbers.Value = Worksheets("F").Range("N2")
...
Else
Dim Drng As Range
Set Drng = Worksheets("F").Range("B4")
...
Drng.End(xlDown).Offset(0, 11).Value = Worksheets("F").Range("N3").Value + 1
...
''3. Click ve listrow moi thao tac
 Dim i As Long
 lrP = Worksheets("F").Range("P" & Rows.Count).End(xlUp).Row
 i = Worksheets("F").Range("Y4:Y" & lrP).Find(What:=txtPreview, LookIn:=xlValues).Row - 4
 ListBox1.Selected(i) = True
...
With Me.ListBox2
    For i = 0 To .ListCount - 1
      If .Selected(i) Then
        ...
        Worksheets("F").Range("AS4") = ListBox2.List(i, 1)
        Worksheets("F").Range("AS5") = ListBox2.List(i, 1)
        Exit For
      End If
    Next
End With
...
ElseIf Me.txtNumber.Value < 10 Then
  ''1.TH txtASub2 va txtAsub1 trong
   If Me.txtASub2.Value = "-" Or Me.txtASub2.Value = "" Then
      ...
      Me.txtPreview.Value = "F_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_0" & Me.txtNumber.Value
      Else
      Me.txtPreview.Value = "F_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtASub1.Value & "_0" & Me.txtNumber.Value
      End If
   Else
   Me.txtPreview.Value = "F_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtASub1.Value & "_" & Me.txtASub2.Value & "_0" & Me.txtNumber.Value
   End If
Else
''B.TH con lai Number>10
   If Me.txtASub2.Value = "-" Or Me.txtASub2.Value = "" Then
      If Me.txtASub1.Value = "-" Or Me.txtASub1.Value = "" Then
      Me.txtPreview.Value = "F_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtNumber.Value
      Else
      Me.txtPreview.Value = "F_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtASub1.Value & "_" & Me.txtNumber.Value
      End If
   Else
   Me.txtPreview.Value = "F_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtASub1.Value & "_" & Me.txtASub2.Value & "_" & Me.txtNumber.Value
   End If
End If
End Sub
...
Worksheets("F").Range("O4").Value = Me.txtPreview.Value
End Sub

Private Sub UserFormF_Initialize()
Worksheets("F").Activate
lrB = Worksheets("F").Range("B" & Rows.Count).End(xlUp).Row
Worksheets("F").Range("B4:M" & lrB).Name = "IndataF"
ListBox1.RowSource = "IndataF"
...
Me.txtPath.Value = Worksheets("F").Range("AV1").Value
Me.cboUndo.Enabled = False
...
Private Sub UserForm_QueryClose(Cancel As Integer, CloseMode As Integer)
 ...
 Application.ScreenUpdating = False
    Worksheets("F").Activate
    Worksheets("F").Columns("B:M").Select
    Selection.Copy
    Worksheets("F").Columns("AC:AN").PasteSpecial xlPasteValues
 ''2. Xoa truong du lieu copy tu Folder doi ten vao
lrAQ = Worksheets("F").Range("AQ" & Rows.Count).End(xlUp).Row
 Worksheets("F").Range("AQ4:AQ" & lrAQ).ClearContents
 Worksheets("F").Range("AS4") = ""
 Worksheets("F").Range("AU4") = ""
     ''Tro ve man hinh chinh
Call FORMAT
Application.ScreenUpdating = True
ThisWorkbook.Sheets("F").Protect Password:="ttdg"
End Sub
Sub FORMAT()
    Worksheets("F").Columns("N:AA").Select
    ...
    With Selection.Font
        ...
        .TintAndShade = 0
    End With
     Worksheets("F").Columns("AP:AV").Select
    Selection.Borders(xlDiagonalDown).LineStyle = xlNone
    Selection.Borders(xlDiagonalUp).LineStyle = xlNone
    Selection.Borders(xlEdgeLeft).LineStyle = xlNone
    Selection.Borders(xlEdgeTop).LineStyle = xlNone
...
'' CATEGORY
    Worksheets("F").Activate
    Worksheets("F").Range("C4:D" & lrB).Copy
    Worksheets("F_Source").Activate
    Worksheets("F_Source").Range("B4").PasteSpecial xlPasteValues
    Dim lrBs As Long
    lrBs = Worksheets("F_Source").Range("B" & Rows.Count).End(xlUp).Row - 1
    Worksheets("F_Source").Range("B4:C" & lrBs).RemoveDuplicates Columns:=Array(1, 2), Header:=xlYes
''TYPE
    Worksheets("F").Activate
    Worksheets("F").Range("E4:F" & lrB).Copy
    Worksheets("F_Source").Activate
    Worksheets("F_Source").Range("E4").PasteSpecial xlPasteValues
    Dim lrEs As Long
    lrEs = Worksheets("F_Source").Range("E" & Rows.Count).End(xlUp).Row - 1
    Worksheets("F_Source").Range("E4:F" & lrEs).RemoveDuplicates Columns:=Array(1, 2), Header:=xlYes
''SUBTYPE 1
    Worksheets("F").Activate
    Worksheets("F").Range("G4:H" & lrB).Copy
    Worksheets("F_Source").Activate
    Worksheets("F_Source").Range("H4").PasteSpecial xlPasteValues
    Dim lrHs As Long
    lrHs = Worksheets("F_Source").Range("H" & Rows.Count).End(xlUp).Row - 1
    Worksheets("F_Source").Range("H4:I" & lrHs).RemoveDuplicates Columns:=Array(1, 2), Header:=xlYes

'''SUBTYPE 2
    Worksheets("F").Activate
    Worksheets("F").Range("I4:J" & lrB).Copy
    Worksheets("F_Source").Activate
    Worksheets("F_Source").Range("K4").PasteSpecial xlPasteValues
    Dim lrKs As Long
    lrKs = Worksheets("F_Source").Range("K" & Rows.Count).End(xlUp).Row - 1
    Worksheets("F_Source").Range("K4:L" & lrHs).RemoveDuplicates Columns:=Array(1, 2), Header:=xlYes
    Worksheets("F").Activate
End Sub
