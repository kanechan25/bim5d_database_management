VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} UserFormM 
   Caption         =   "T&TDG Mechanical Database Family Management"
   ClientHeight    =   10515
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   22920
   OleObjectBlob   =   "UserFormMechanical.frx":0000
...
Private Sub cboCate_Change()
    Worksheets("M").Range("C2") = Me.cboCate.Value
    Range("D2").Select
    ActiveCell.FormulaR1C1 = "=IFERROR(VLOOKUP(RC[-1],R[2]C[-1]:R[1000]C,2,FALSE),"""")"
    Me.txtACate.Value = Worksheets("M").Range("D2")
End Sub
...
Private Sub cboSub1_Change()
    Worksheets("M").Range("G2") = Me.cboSub1.Value
    Range("H2").Select
    ActiveCell.FormulaR1C1 = _
        "=IFERROR(VLOOKUP(RC[-1],R[2]C[-1]:R[1000]C,2,FALSE),"""")"
    Me.txtASub1.Value = Worksheets("M").Range("H2")
End Sub

Private Sub cboSub2_Change()
    Worksheets("M").Range("I2") = Me.cboSub2.Value
    Range("J2").Select
    ActiveCell.FormulaR1C1 = _
        "=IFERROR(VLOOKUP(RC[-1],R[2]C[-1]:R[1000]C,2,FALSE),"""")"
    Me.txtASub2.Value = Worksheets("M").Range("J2")
End Sub

Private Sub cboType_Change()
    Worksheets("M").Range("E2") = Me.cboType.Value
    Range("F2").Select
    ActiveCell.FormulaR1C1 = _
        "=IFERROR(VLOOKUP(RC[-1],R[2]C[-1]:R[1000]C,2,FALSE),"""")"
    Me.txtAType.Value = Worksheets("M").Range("F2")
End Sub
...
'lrAO = Worksheets("M").Range("AQ" & Rows.Count).End(xlUp).Row
'Set findvalue = Sheets("M").Range("AQ4:AQ" & lrAQ).Find(What:=Me.txtEditFile, LookIn:=xlValues)
If Me.txtEditFile.Text = "" Or Me.ListBox2 = "" Then
   Exit Sub
Else
   Dim OldFile As String
   Dim NewFile As String
   'Worksheets("M").Range("AS5").Value = Me.txtEditFile.Text
    OldFile = Worksheets("M").Range("AV5")
    NewFile = Worksheets("M").Range("AV4")
Existed:
    If Worksheets("M").Range("AV4").Value = Worksheets("M").Range("AV5").Value Then
...
Else
  ...
  lrP = Worksheets("M").Range("P" & Rows.Count).End(xlUp).Row
  i = Worksheets("M").Range("Y4:Y" & lrP).Find(What:=Me.txtName, LookIn:=xlValues).Row - 4
Call del
End If
...
Dim findvalue As Range
lrB = Worksheets("M").Range("B" & Rows.Count).End(xlUp).Row
Set findvalue = Worksheets("M").Range("K4:K" & lrB).Find(What:=Me.txtName, LookIn:=xlValues)
findvalue.Value = ""
...
'1. SortData
   lrB = Worksheets("M").Range("B" & Rows.Count).End(xlUp).Row
    ...
    ActiveWorkbook.Worksheets("M").Sort.SortFields.Add Key:=Range("B4:B" & lrB), _
        SortOn:=xlSortOnValues, Order:=xlDescending, DataOption:=xlSortNormal
    
    ActiveWorkbook.Worksheets("M").Sort.SortFields.Add Key:=Range("K4:K" & lrB), _
  ...
  ''''2. Sort Outdata
  lrP = Worksheets("M").Range("P" & Rows.Count).End(xlUp).Row
    Worksheets("M").Range("P3:AA" & lrP).Select
    ActiveWorkbook.Worksheets("M").Sort.SortFields.Clear
    ActiveWorkbook.Worksheets("M").Sort.SortFields.Add Key:=Range("P4:P" & lrP), _
        SortOn:=xlSortOnValues, Order:=xlDescending, DataOption:=xlSortNormal
    
    ActiveWorkbook.Worksheets("M").Sort.SortFields.Add Key:=Range("Y4:Y" & lrP), _
        SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    With ActiveWorkbook.Worksheets("M").Sort
...
''*xet xem da co name bi trung
  ...
  lrB = Worksheets("M").Range("B" & Rows.Count).End(xlUp).Row
  Set findName = Worksheets("M").Range("K4:K" & lrB).Find(What:=Me.txtName, LookIn:=xlValues)
...
Dim findvalue As Range
lrB = Worksheets("M").Range("B" & Rows.Count).End(xlUp).Row
Set findvalue = Sheets("M").Range("M4:M" & lrB).Find(What:=Me.txtID, LookIn:=xlValues)
...
Call cmdSearch_Click
 ...
 lrP = Worksheets("M").Range("P" & Rows.Count).End(xlUp).Row
 On Error Resume Next
 i = Worksheets("M").Range("Y4:Y" & lrP).Find(What:=Me.txtPreview, LookIn:=xlValues).Row - 4
...
Else
    Dim findvalue As Range
  On Error GoTo Existed
    lrAQ = Worksheets("M").Range("AQ" & Rows.Count).End(xlUp).Row
    ...
    Else
      Dim OldFile As String
      Dim NewFile As String
      Worksheets("M").Range("AU4").Value = Me.txtEditFile.Value 'dien ten file moi
      Worksheets("M").Range("AU5").Value = Me.txtEditFile.Value 'dien ten file moi
      OldFile = Worksheets("M").Range("AV2")
      NewFile = Worksheets("M").Range("AV3")
Existed:
    If Worksheets("M").Range("AV2").Value = Worksheets("M").Range("AV3").Value Then
...
''Xoa Get list file cu
    Worksheets("M").Range("AQ:AQ").ClearContents
    Dim myPath As String
    myPath = Me.txtPath.Text
    If myPath <> "" Then
...
'*Sort va Filter du lieu trc khi dien
    ...
    lrAQ = Worksheets("M").Range("AQ" & Rows.Count).End(xlUp).Row
    ActiveWorkbook.Worksheets("M").Sort.SortFields.Clear
    ActiveWorkbook.Worksheets("M").Sort.SortFields.Add Key:=Range("AQ4:AQ" & lrAQ), _
        SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    With ActiveWorkbook.Worksheets("M").Sort
    ...
    'ActiveWorkbook.Worksheets("M").Sort.SortFields.Add2 Key:=Range("AQ4"), _
        'SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
'*Dien du lieu vao Listrow
   ListBox2.List = Worksheets("M").Range("AP4:AQ" & lrAQ).Value
...
End If
'MsgBox (Get_Path)
Worksheets("M").Cells(1, 48).Value = get_Path
...
On Error GoTo errHandler:
 ...
 Worksheets("M").Range("O2") = Me.cboSelect.Value
 Worksheets("M").Range("O3") = Me.txtSearch.Value
   Dim lr1 As Long
   lr1 = Worksheets("M").Cells(Rows.Count, "B").End(xlUp).Row
 Worksheets("M").Range("B3:M" & lr1).AdvancedFilter Action:=xlFilterCopy, _
 CriteriaRange:=Range("M!Criteria"), _
 CopyToRange:=Range("M!Extract"), Unique:=False
 ''tim dong cuoi cua du lieu ket qua de dien vao listrow
lrP = Worksheets("M").Range("P" & Rows.Count).End(xlUp).Row
 ListBox1.RowSource = Worksheets("M").Range("P4:AA" & lrP).Address
 Me.txtMax.Value = Worksheets("M").Range("N1")
 Me.txtNumbers.Value = Worksheets("M").Range("N2")
...
Else
Dim Drng As Range
Set Drng = Worksheets("M").Range("B4")
...
Drng.End(xlDown).Offset(0, 11).Value = Worksheets("M").Range("N3").Value + 1
...
With Me.ListBox2
    For i = 0 To .ListCount - 1
      If .Selected(i) Then
        ...
        Worksheets("M").Range("AS4") = ListBox2.List(i, 1)
        Worksheets("M").Range("AS5") = ListBox2.List(i, 1)
        Exit For
      End If
    Next
End With
...
ElseIf Me.txtNumber.Value < 10 Then
  ''1.TH txtASub2 va txtAsub1 trong
   If Me.txtASub2.Value = "-" Or Me.txtASub2.Value = "" Then
      ...
      Me.txtPreview.Value = "M_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_0" & Me.txtNumber.Value
      Else
      Me.txtPreview.Value = "M_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtASub1.Value & "_0" & Me.txtNumber.Value
      End If
   Else
   Me.txtPreview.Value = "M_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtASub1.Value & "_" & Me.txtASub2.Value & "_0" & Me.txtNumber.Value
   End If
Else
''B.TH con lai Number>10
   If Me.txtASub2.Value = "-" Or Me.txtASub2.Value = "" Then
      If Me.txtASub1.Value = "-" Or Me.txtASub1.Value = "" Then
      Me.txtPreview.Value = "M_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtNumber.Value
      Else
      Me.txtPreview.Value = "M_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtASub1.Value & "_" & Me.txtNumber.Value
      End If
   Else
   Me.txtPreview.Value = "M_" & Me.txtACate.Value & "_" & Me.txtAType.Value & "_" & Me.txtASub1.Value & "_" & Me.txtASub2.Value & "_" & Me.txtNumber.Value
   End If
End If
End Sub
...
Worksheets("M").Range("O4").Value = Me.txtPreview.Value
End Sub
...
Private Sub UserFormM_Initialize()
Worksheets("M").Activate
lrB = Worksheets("M").Range("B" & Rows.Count).End(xlUp).Row
Worksheets("M").Range("B4:M" & lrB).Name = "IndataM"
ListBox1.RowSource = "IndataM"

End Sub

Private Sub UserForm_Activate()
'KHOI DONG DIEN SAN LINK PATH
Me.txtPath.Value = Worksheets("M").Range("AV1").Value
Me.cboUndo.Enabled = False
...
Application.ScreenUpdating = False
 ''1. Copy du lieu sang khu vuc khac
    Worksheets("M").Columns("B:M").Select
    Selection.Copy
    Worksheets("M").Columns("AC:AN").PasteSpecial xlPasteValues
 ''2. Xoa truong du lieu copy tu Folder doi ten vao
 lrAQ = Worksheets("M").Range("AQ" & Rows.Count).End(xlUp).Row
 Worksheets("M").Range("AQ4:AQ" & lrAQ).ClearContents
 Worksheets("M").Range("AS4") = ""
 Worksheets("M").Range("AU4") = ""
     ''Tro ve man hinh chinh
Call FORMAT
Application.ScreenUpdating = True
ThisWorkbook.Sheets("M").Protect Password:="ttdg"
End Sub

Sub FORMAT()
    Worksheets("M").Columns("N:AA").Select
    ...
    With Selection.Font
        ...
        .TintAndShade = 0
    End With
     Worksheets("M").Columns("AP:AV").Select
    Selection.Borders(xlDiagonalDown).LineStyle = xlNone
    Selection.Borders(xlDiagonalUp).LineStyle = xlNone
    Selection.Borders(xlEdgeLeft).LineStyle = xlNone
    Selection.Borders(xlEdgeTop).LineStyle = xlNone
...
'' CATEGORY
    Worksheets("M").Activate
    Worksheets("M").Range("C4:D" & lrB).Copy
    Worksheets("M_Source").Activate
    Worksheets("M_Source").Range("B4").PasteSpecial xlPasteValues
    Dim lrBs As Long
    lrBs = Worksheets("M_Source").Range("B" & Rows.Count).End(xlUp).Row - 1
    Worksheets("M_Source").Range("B4:C" & lrBs).RemoveDuplicates Columns:=Array(1, 2), Header:=xlYes
''TYPE
    Worksheets("M").Activate
    Worksheets("M").Range("E4:F" & lrB).Copy
    Worksheets("M_Source").Activate
    Worksheets("M_Source").Range("E4").PasteSpecial xlPasteValues
    Dim lrEs As Long
    lrEs = Worksheets("M_Source").Range("E" & Rows.Count).End(xlUp).Row - 1
    Worksheets("M_Source").Range("E4:F" & lrEs).RemoveDuplicates Columns:=Array(1, 2), Header:=xlYes
''SUBTYPE 1
    Worksheets("M").Activate
    Worksheets("M").Range("G4:H" & lrB).Copy
    Worksheets("M_Source").Activate
    Worksheets("M_Source").Range("H4").PasteSpecial xlPasteValues
    Dim lrHs As Long
    lrHs = Worksheets("M_Source").Range("H" & Rows.Count).End(xlUp).Row - 1
    Worksheets("M_Source").Range("H4:I" & lrHs).RemoveDuplicates Columns:=Array(1, 2), Header:=xlYes

'''SUBTYPE 2
    Worksheets("M").Activate
    Worksheets("M").Range("I4:J" & lrB).Copy
    Worksheets("M_Source").Activate
    Worksheets("M_Source").Range("K4").PasteSpecial xlPasteValues
    Dim lrKs As Long
    lrKs = Worksheets("M_Source").Range("K" & Rows.Count).End(xlUp).Row - 1
    Worksheets("M_Source").Range("K4:L" & lrHs).RemoveDuplicates Columns:=Array(1, 2), Header:=xlYes
    Worksheets("M").Activate
End Sub
